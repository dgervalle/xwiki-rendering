...
public class DefaultRenderingContext implements MutableRenderingContext
{
    ...
    private static final class Context implements Cloneable
    {
         ...
         */
        private Context(Transformation transformation, XDOM xdom, Block block, Syntax syntax,
            String id, boolean restricted)
        {
            this.id = id;
            this.xdom = xdom;
            this.block = block;
    ...
    @Override
    public void push(Transformation transformation, TransformationContext context, Block block)
    {
        push(transformation, context.getXDOM(), block, context.getSyntax(), context.getId(), context.isRestricted());
    }

    @Override
    public void push(Transformation transformation, XDOM xdom, Block block, Syntax syntax, String id,
        boolean restricted)
    {
        Deque<Context> stack = getContextstack(true);
        if (stack != null) {
            stack.push(new Context(transformation, xdom, block, syntax, id, restricted));
        }
    }
    ...
    {
        try {
            push(transformation, context, block);
